...
export default function PatientsTab({
  ...
  // Enrollment ID inline edit
  const [editingEnrollId, setEditingEnrollId] = useState(false);
  const [enrollIdDraft, setEnrollIdDraft] = useState('');
  ...
  const filteredPatients = patients.filter(p => {
    const matchesSearch = p.name.toLowerCase().includes(searchTerm.toLowerCase()) || 
                          ...
                          (p.enrollmentId && p.enrollmentId.toLowerCase().includes(searchTerm.toLowerCase())) ||
              ...
              return (
                ...
                >
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <span className="block text-[8px] opacity-60 font-sans mt-0.5">ID: {p.enrollmentId || p.id}</span>
                  </div>
                </button>
              );
            })
            ...
            <div className="p-6 bg-slate-900 text-white flex flex-col md:flex-row justify-between items-start md:items-center gap-4" id="patient-detail-header">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1.5" id="enrollment-id-row">
                    <span className="text-[10px] uppercase font-bold text-slate-500 font-sans tracking-tight">Enrollment ID:</span>
                    {editingEnrollId ? (
                      <span className="flex items-center gap-1.5">
                        <input
                          ...
                          type="text"
                          autoFocus
                          className="bg-slate-800 border border-slate-600 text-white text-[11px] font-mono rounded-lg px-2 py-1 w-40 focus:outline-none focus:border-emerald-400"
                          value={enrollIdDraft}
                          onChange={e => setEnrollIdDraft(e.target.value)}
                          onKeyDown={e => {
                            if (e.key === 'Enter') {
                              onUpdatePatient(selectedPatient.id, { enrollmentId: enrollIdDraft.trim() });
                              setEditingEnrollId(false);
                        ...
                        />
                        <button
                          id="save-enroll-id-btn"
                          onClick={() => {
                            onUpdatePatient(selectedPatient.id, { enrollmentId: enrollIdDraft.trim() });
                            setEditingEnrollId(false);
                          }}
                          className="text-[10px] font-sans font-bold bg-emerald-600 hover:bg-emerald-700 text-white px-2 py-1 rounded-lg"
                        >
                          Save
                        </button>
                        <button
                          id="cancel-enroll-id-btn"
                          onClick={() => setEditingEnrollId(false)}
                          className="text-[10px] font-sans text-slate-400 hover:text-slate-200 px-1"
                        >
                          Cancel
                        </button>
                      </span>
                      ...
                      <button
                        id="edit-enroll-id-btn"
                        onClick={() => {
                          setEnrollIdDraft(selectedPatient.enrollmentId || selectedPatient.id);
                          setEditingEnrollId(true);
                        }}
                        className="flex items-center gap-1 text-[11px] font-mono text-emerald-300 hover:text-emerald-200 transition"
                        title="Click to edit enrollment ID"
                      >
                        {selectedPatient.enrollmentId || selectedPatient.id}
                        <Edit size={11} className="opacity-70" />
                      </button>
                    )}
                  </div>
                </div>
